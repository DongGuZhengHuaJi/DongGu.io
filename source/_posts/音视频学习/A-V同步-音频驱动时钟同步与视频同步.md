...
4. **pause/resume** 通过冻结/恢复 Clock 和 AudioOutput 实现，解码线程不停，保证恢复时数据已就绪
5. **seek** 按严格顺序停止并重启整个 pipeline，用 `requestClockReset()` 让 AudioOutput 在首帧重新对齐时钟

音频是"推"时钟的一方，视频是"跟"时钟的一方。人耳对音频抖动敏感，同时时钟由声卡晶振驱动、极其稳定；人眼对偶尔的重帧/跳帧容忍度较高，所以视频侧用丢帧和等待来追赶。

下一篇将展开视频渲染管线 —— 如何将 YUV 数据通过 OpenGL 渲染到屏幕上。
